...
export default function TopCreatorsSection({ creators = [], loading }) {
  ...
  const top5 = Array.isArray(creators) ? creators.slice(0, 5) : [];
  ...
  return (
    <section className="mb-8">
      ...
      {!loading && top5.length > 0 && (
          ...
          {top5.map((c, index) => (
            ...
            >
              <div className="relative">
                <img
                  src={c.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(String(c.name || c.id || ''))}`}
                  alt={c.name ?? 'Creator'}
              ...
              <span className="text-xs text-gray-500">
                {c.followers != null && c.followers !== ''
                  ? (typeof c.followers === 'number' ? `${(c.followers / 1000).toFixed(1)}K followers` : `${c.followers} followers`)
                  : ''}
                {c.followers != null && c.followers !== '' && (c.videosCount != null || c.videos_count != null) ? ' · ' : ''}
                {(c.videosCount ?? c.videos_count ?? 0)} videos
              </span>
            </button>
          ))}
        </div>
        ...
        {!loading && top5.length === 0 && (
        <p className="text-gray-500 text-sm py-4">No creators available.</p>
      )}
    </section>
  );
}
